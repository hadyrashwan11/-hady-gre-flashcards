...
const STORE="hadyGreProgressV4",SESSION_STORE="hadyGreSessionV1",RECENT_STORE="hadyGreRecentV1";
let state=JSON.parse(localStorage.getItem(STORE)||"{}");
let recent=[];
try{recent=JSON.parse(localStorage.getItem(RECENT_STORE)||"[]")}catch{}
...
let mode=["all","learned","new","learning","starting","known","unseen"].includes(savedSession.mode)?savedSession.mode:"all";
...
function deckFor(m){
  ...
  if(m==="starting") return DATA.filter(x=>state[x.w]==="starting");
...
function render(){
  ...
  if(!deck.length){
    ...
    $("definition").textContent=mode==="learning"?"Mark words Still learning and they will collect here.":mode==="starting"?"Mark words Starting to remember and they will collect here.":"Try another filter.";
    $("counter").textContent="";
    $("frontStatus").textContent="";
    $("markKnown").textContent="I know this";
    $("markStarting").textContent="Starting to remember";
  ...
  $("frontStatus").textContent=s==="known"?"Known":s==="starting"?"Starting to remember":s==="learning"?"Still learning":c.g==="learned"?"Already learned":"New word";
  $("markKnown").textContent=s==="known"?"✓ Known":"I know this";
  $("markStarting").textContent=s==="starting"?"✓ Starting to remember":"Starting to remember";
  $("markLearning").textContent=s==="learning"?"✓ Still learning":"Still learning";
  $("markKnown").setAttribute("aria-pressed",s==="known"?"true":"false");
  $("markStarting").setAttribute("aria-pressed",s==="starting"?"true":"false");
...
function recordRecent(word){
  ...
  localStorage.setItem(RECENT_STORE,JSON.stringify(recent));
}
...
$("markKnown").onclick=()=>{
  ...
  recordRecent(w);
  save();
  if(mode==="learning"||mode==="starting"||mode==="unseen"){deck.splice(index,1);if(index>=deck.length)index=Math.max(0,deck.length-1);render()}else next();
};
$("markLearning").onclick=()=>{
  ...
  recordRecent(w);
  save();
  if(mode==="known"||mode==="starting"||mode==="unseen"){deck.splice(index,1);if(index>=deck.length)index=Math.max(0,deck.length-1);render()}else next();
};
$("markStarting").onclick=()=>{
  if(!deck.length)return;
  const w=deck[index].w;
  state[w]="starting";
  recordRecent(w);
  save();
  if(mode==="known"||mode==="learning"||mode==="unseen"){deck.splice(index,1);if(index>=deck.length)index=Math.max(0,deck.length-1);render()}else next();
};
$("session25").onclick=()=>{
  let p=DATA.filter(x=>state[x.w]==="learning");
  p=[...p,...DATA.filter(x=>state[x.w]==="starting"),...DATA.filter(x=>!state[x.w]&&x.g==="new"),...DATA.filter(x=>!state[x.w]&&x.g==="learned")];
...
$("copyRecent").onclick=()=>{
  const words=recent.slice(-20);
  copyText(words.join(", "),$("copyRecent"),"Copy these words:");
};
$("copyReview").onclick=()=>{
  const learning=DATA.filter(x=>state[x.w]==="learning").map(x=>x.w);
  const starting=DATA.filter(x=>state[x.w]==="starting").map(x=>x.w);
  const text="Still learning: "+(learning.length?learning.join(", "):"None")+"\nStarting to remember: "+(starting.length?starting.join(", "):"None");
  copyText(text,$("copyReview"),"Copy your review list:");
};
document.querySelectorAll("[data-mode]").forEach(b=>b.onclick=()=>setMode(b.dataset.mode));
$("search").oninput=e=>{
  const q=e.target.value.trim().toLowerCase();
  if(!q){setMode(["all","learned","new","learning","starting","known","unseen"].includes(mode)?mode:"all");return}
...
for(const el of [$("scene"),$("next"),$("prev"),$("markKnown"),$("markStarting"),$("markLearning"),$("session25"),$("copyRecent"),$("copyReview")]){
